...
import com.softech.ls360.lcms.contentbuilder.utils.AuthConstant;
import com.softech.ls360.lcms.contentbuilder.utils.LCMSProperties;


public class ActiveDirectoryServiceImpl implements IActiveDirectoryService{

	private static final Logger log = Logger.getLogger(ActiveDirectoryServiceImpl.class.getName());
	
	private static final String adminDN = LCMSProperties.getLCMSProperty("ad.admin.fulldn");
	private static final String adminUserId = LCMSProperties.getLCMSProperty("ad.admin.cn");
	private static final String adminPassword = LCMSProperties.getLCMSProperty("ad.admin.password");
	private static final String ldapsURL = LCMSProperties.getLCMSProperty("ad.ldap.url");
	private static final String ldapURL = LCMSProperties.getLCMSProperty("ad.ldapa.url");
	private static final String keyStore = LCMSProperties.getLCMSProperty("ad.certs.keystore");
	private static final String timeout = LCMSProperties.getLCMSProperty("ad.connect.timeout");
	private static final String AD_FIELD_isLCMS = "eXTLCMS";
	private static final boolean adEnabled = Boolean.valueOf(LCMSProperties.getLCMSProperty("ad.integration.enabled"));
	private static final String baseDN = LCMSProperties.getLCMSProperty("ad.user.basedn");
	private static final boolean adLoggingEnabled = Boolean.valueOf(LCMSProperties.getLCMSProperty("ad.timelogging.enabled"));
	...
	public ActiveDirectoryUser updateUser(VU360User userToUpdate){
		...
		try{
			...
			distinguishedName = getADUserDN(userToUpdate.getUsername(), false);
	...
	public  void addAttribute(String userToUpdate){	 
		...
		try{
			logForAD(String.format("Begin Updating user: %s", userToUpdate));
			System.out.println(String.format("Locating the user first in AD: %s", userToUpdate));			
			distinguishedName = getADUserDN(userToUpdate, false);
			
			if (distinguishedName.length()==0){
				System.out.println(String.format("User not found in entire AD. Cannot Update user:%s in AD.", userToUpdate));
				return;
			}
			
			try{
				ctx =  getLDAPContext(false, adminDN, adminPassword);
 				BasicAttribute atr = new BasicAttribute(AD_FIELD_isLCMS);
				atr.add("TRUE");
				
				ModificationItem[] modifiedEntries = new ModificationItem[1];
				modifiedEntries[0] = new ModificationItem(DirContext.REPLACE_ATTRIBUTE, atr);

				ctx.modifyAttributes(distinguishedName, modifiedEntries);
				System.out.println("User updated in AD Successfully");
			}
			catch (Exception ee ){
				System.out.print(ee.getMessage() + ee);
				return;
			}		
			finally {
				if(ctx!=null) ctx.close();	
		...
		logForAD(String.format("Finished Updated user: %s" , userToUpdate));
	...
	public ActiveDirectoryUser addUser(VU360User newUser){
		...
		try{
			...
			String existingUser = getADUserDN(newUser.getUsername(), true);			
		...
		if(userName!=null && password!=null){
			try{
				...
				try{
					...
					ctx = getLDAPContext(true, distinguishedName, password);
				...
				if (!isAuthenticated){
					try{
						//Setting parameters for Context
						distinguishedName2 = getADUserDN(userName, true);
	...
	public boolean findADUser(String username) {
		String userDN = getADUserDN(username, true);
		return userDN.length()>0? true : false;
	}

	private String getADUserDN(String username, boolean ssl){
		LdapContext ctx = null;

		try{
			logForAD(String.format("Begin Find user: %s" , username));
			try{
				ctx =  getLDAPContext(ssl, adminDN, adminPassword);
